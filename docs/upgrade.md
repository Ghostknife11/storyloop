...
错误码，产物布局、CLI 与 `metadata.json` 契约和 1.7.1 逐字一致。新增的是** Run 级可观测性**：
每次 Run 多一份 `telemetry.json`，一条只读路由，一个前端面板，以及实验汇总里一组效率指标。

要紧的有五条：

1. **Run 根目录多一个文件**：`telemetry.json`。运行级固定文件数从十个变成十一个；
   `attempts/` 与 `repairs/` 下一份都没多。按「运行级正好 N 个文件」硬编码的工具要改口径——
   更稳的做法是忽略不认识的文件名，而不是数个数。字段级契约见
   [telemetry.md](./telemetry.md)。
2. **新增一条只读路由**：`GET /api/runs/<run_id>/telemetry` → `{telemetry: ...}`。
   如果下游对 `/api` 做了白名单代理，现在要把这个路径放进去。
   它不会调用模型、不会写任何产物。
3. **两个响应各多一个可选字段**：`GET /api/runs/<run_id>` 多一个 `telemetry`（与上面同一份
   内容，顺手带上）；实验汇总里每个 variant 多一个 `efficiency` 对象（七项
   `{mean, sampleCount}`）。请把它们当可选字段处理，不要假设一定存在。按字段穷举的解析器
   （`Object.keys` 比对、`strict` 反序列化）现在会多出几个可能是 `null` 的键。
4. **旧 Run 没有遥测，读出来是 `null`，磁盘上不会被补写。** 1.8.0 之前生成的 Run：
   路由返回 `{ "telemetry": null }`（HTTP 200，不是 404），Run 详情的 `telemetry` 是 `null`，
   前端面板整个隐藏。实验里 1.8.0 之前跑出来的那格，`efficiency` 各项是
   `{mean: null, sampleCount: 0}`——「没有数据」不会被显示成 0。
5. **遥测不改变任何生成行为。** 它只记录：各阶段耗时、LLM 调用、Retry / Repair 次数、
   失败阶段、真实可得的 token / cost。没有任何代码读遥测来决定重试、修订或接受判定，
   也没有失败归因、根因分析、告警、分布式追踪或基准排名——这些都不在这一版里。

```bash
git fetch && git checkout 1.8.0     # tag 不带 v 前缀
npm install
```

回滚到 1.7.1 的代价为零：多出来的 `telemetry.json`、一个路由与两个响应字段被旧版本忽略，
1.7.1 读 1.8.0 跑出来的 Run 与往常一样。新增的 `tests/test_telemetry.test.ts` 在回滚后会红，
这正是它要挡住的事。
...
**没有任何需要改代码的地方。** 1.7.1 是对 1.7.0 的审查修订：三个刚发布就发现的问题、
外加一处文档订正。没有新路由、没有新错误码、没有删字段、没有改字段名、没有改行为口径。
已经在跑的实验定义与结果文件 1.7.1 照读。

要紧的有四条：

1. **样本行现在有分数了**：`GET /api/experiments/<id>` 与 `POST .../run` 返回的
   `result.runs` 每条引用上多两个键——`overallScore` 与 `commercialScore`，读自这条样本
   自己的 `quality.json` / `commercial-review.json`。1.7.0 声明了却从不写它们，于是每一行
   样本都显示「整体 — · 商业 —」。1.7.0 已经跑完的实验结果文件里没有这两个键，读到的就是
   `null`，界面照老样子显示 `—`——旧数据不会被补写。
2. **`result.runs` 的消费者要当新键不存在**：如果你的解析器按字段穷举（`Object.keys` 比对、
   `strict` 反序列化），现在会多出两个可能是 `null` 的键。把它当可选字段处理。
3. **多了一种 409**：`EXPERIMENT_CONFLICT` 从此还有一个含义——这个实验此刻正在运行中。
   1.7.0 里两个并发请求会双双通过预检，把整个实验跑两遍（两倍付费请求）。现在第二条请求
   直接 409。这把锁是**进程内**的：多 worker / 多实例部署下每个进程各有自己的锁，
   重复跑实验在那种部署里仍可能发生，靠的是「`results.json` 存在就 409」这道原有闸门兜底。
4. **不像目录名的 id 一律 404**：`exp/../../x` 这类输入以前会变成 500，现在是
   404 `EXPERIMENT_NOT_FOUND`。如果你下游把 500 当作「服务出事要重试」，
   这条改变让错误码回到正确的一类。

```bash
git fetch && git checkout 1.7.1     # tag 不带 v 前缀
npm install
```

回滚到 1.7.0 的代价为零：多出来的两个分数键被旧版本忽略，锁与 id 闸门是收紧行为而非
新契约。新增的 `tests/test_experiment_run_guard.test.ts` 在回滚后会红，这正是它要挡住的事。
...
从 1.8.0 回滚到 1.7.1 同样不需要迁移：多出来的 `telemetry.json`、一条路由与两个响应字段
被旧版本忽略，1.7.1 读 1.8.0 的 Run 与往常一样，只是重新变成「看不出这次跑了多久、
调了几次模型、在哪一步失败」。
