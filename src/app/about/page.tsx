...
export default function AboutPage() {
  ...
  useEffect(() => {
    fetch("/api/version").then(r => r.json()).then(d => { if (d?.version) setVersion(d.version); }).catch(() => setVersion("0.5.0"));
  }, []);
    ...
    <div className="h-full overflow-auto p-4 sm:p-6">
      <div className="mx-auto max-w-2xl space-y-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">一个具备剧情规划、正文生成、Run 持久化和基础自动审阅能力的 AI 短篇小说生成器</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur p-4 sm:p-5 text-xs text-muted-foreground leading-5">
          本项目按版本逐步公开演进。当前版本已包含提示词工程、StoryConfig 复用、剧情规划、正文生成、基于 Run 的生成流水线与基础自动审阅（总分 / 摘要 / 优点 / 问题）；审阅只提供反馈，不会自动重新生成或修复正文。多维评审、质量校验、自动修复、质量重试、实验、基准与自适应生成尚未包含在本版本中，将在后续版本逐步引入。
        </div>
