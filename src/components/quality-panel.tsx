      ...
      <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1 text-[12px]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <dd className="text-foreground">{state.acceptedText}</dd>
          </div>
          <div className="flex items-center gap-1.5">
            <dt className="text-muted-foreground">Issues</dt>
            <dd className="text-foreground tabular-nums">{state.issues.length}</dd>
          </div>
          <div className="flex items-center gap-1.5">
            <dt className="text-muted-foreground">Suggestions</dt>
            <dd className="text-foreground tabular-nums">{state.suggestions.length}</dd>
          </div>
        </dl>
      </div>

      {state.summary && (
        <p className="mt-2.5 text-[13px] leading-6 text-foreground">{state.summary}</p>
      )}
        ...
        <div className="space-y-1.5">
          ...
          <ul className="space-y-1">
            ...
            {state.issues.map((issue) => (
              <li key={issue.id} className="flex gap-1.5 text-[12px] leading-5 text-foreground">
        ...
        <div className="space-y-1.5">
          ...
          <ul className="space-y-1">
            ...
            {state.suggestions.map((s) => (
              <li key={s.id} className="flex gap-1.5 text-[12px] leading-5 text-foreground">
